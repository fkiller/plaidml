...
class gatherBuilder {
 public:
  explicit gatherBuilder(Tensor x, Tensor y, uint64_t axis, InterpolationMode interpolation_mode,
                         NearestMode nearest_mode, PaddingMode padding_mode, float cube_coeff)
      : x_(x),
        y_(y),
        axis_(axis),
  ...
  Tensor build(edsl_source_location loc = edsl_source_location::current()) const {
    std::vector<Tensor> args = {
        ...
        Tensor(static_cast<uint64_t>(padding_mode_)),        //
    };
 ...
 private:
  ...
  PaddingMode padding_mode_ = PaddingMode::EDGE;
...
class gather {
 public:
  ...
  operator Tensor() {
    auto builder = gatherBuilder(x_, y_, axis_, interpolation_mode_, nearest_mode_, padding_mode_, cube_coeff_);
