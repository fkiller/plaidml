...
struct GatherOpConversion : public OpConversionPattern<tile::GatherOp> {
        ...
        if (elementType.isa<IntegerType>()) {
          ...
          case InterpolationMode::cubic:
            interpVal = buildCubicInterpolationOps(
                ...
                op.cubeCoeffAttr().getValueAsDouble(), op.paddingMode());
            break;
          default:
  ...
  Value buildNearestInterpolationOps(Location loc,
    ...
    result = processPaddingMode(loc, rewriter, result, idx, bounds, elementType,
                                paddingMode);
    return result;
  }
  ...
  Value processPaddingMode(Location loc, ConversionPatternRewriter &rewriter,
                           Value result, Value idx, IndexBounds bounds,
                           Type elementType, PaddingMode paddingMode) const {
    switch (paddingMode) {
